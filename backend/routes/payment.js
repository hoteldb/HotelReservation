...
const db = require('../server.js'); // DB 연결 파일
...
router.get('/payment-details', async (req, res) => {
    ...
    try {
      // 로직 처리
      const [roomDetails] = await req.db.query(
      ...
      const [userDetails] = await req.db.query(
      ...
      const [paymentMethods] = await req.db.query(
        `SELECT payment_method_id AS id, method_name AS name
         FROM payment_method`
      );

      const [bookingDetails] = await req.db.query(
